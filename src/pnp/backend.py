...
class BackendService:
    def __init__(self):
        ...
        interval = self.manager.config_manager.config.get(
            'poll_interval_ms', 2000
        )

        self.steam_detector = SteamDetector(interval_ms=interval)
        self.steam_detector.steam_status_changed.connect(
            self._on_steam_status_changed
        )
        self.steam_detector.game_status_changed.connect(
            self._update_combined_game_status
        )
        ...
        self._poll_timer.timeout.connect(
            self._update_combined_game_status_poll
        )
    ...
    def _update_combined_game_status_poll(self):
        active = self.game_detector.scan_procs()
        self._update_combined_game_status(active)

    def _update_combined_game_status(self, active):
        handover_enabled = self.manager.config_manager.config.get(
            'steam_handover_enabled', True
        )

        steam_running = self.steam_detector.is_steam_running
        game_present = self.steam_detector.is_game_active or active
        steam_input_ready = self.steam_detector.steam_input_active

        should_handover = (
            handover_enabled and steam_running and
            game_present and steam_input_ready
        )

        logger.debug(
            f"Handover eval: Steam={steam_running}, Game={game_present}, "
            f"SI_Ready={steam_input_ready} -> Handover={should_handover}"
        )
        self.manager.set_game_active(should_handover)

        downloader_enabled = self.manager.config_manager.config.get(
                    ...
                    with open(f'/proc/{pid}/environ', 'rb') as f:
                        ...
                        if b'SteamAppId=' in env:
                            ...
                            if match:
                                ...
                                config = (
                                    self.profile_downloader.get_best_config(
                                        appid
                                    )
                                )
                                if config:
                                    self.profile_downloader.apply_config(
                                        appid, config
                                    )
                                    self.profile_downloader.trigger_steam_reload()  # noqa: E501
                                return
                except Exception:
                    continue
        except Exception:
    ...
    def run(self):
        ...
        if missing:
            logger.error(
                f"Missing system dependencies: {', '.join(missing)}. Exiting."
            )
            return
