...
class ConfigManager:
    ...
    def load_defaults(self):
        self.config = {
            ...
            "mapping": {
                "axismap": "-y1=y1,-y2=y2",
                "absmap": "ABS_X=x1,ABS_Y=y1,ABS_RX=x2,ABS_RY=y2,ABS_Z=lt,"
                          "ABS_RZ=rt,ABS_HAT0X=dpad_x,ABS_HAT0Y=dpad_y",
                "keymap": "BTN_SOUTH=a,BTN_EAST=b,BTN_NORTH=x,BTN_WEST=y,"
                          "BTN_TL=lb,BTN_TR=rb,BTN_TL2=lt,BTN_TR2=rt,"
                          "BTN_THUMBL=tl,BTN_THUMBR=tr,BTN_SELECT=back,"
                          "BTN_START=start,BTN_MODE=guide"
            }
        }
            ...
            try:
                ...
                if 'settings' in parser:
                    ...
                    self.config["poll_interval_ms"] = int(
                        s.get('polling_interval_ms', 2000)
                    )
                    self.config["rumble_gain"] = s.get('rumble_gain', '15%')

                if 'mapping' in parser:
                    m = parser['mapping']
                    for key in ['axismap', 'absmap', 'keymap']:
                        self.config['mapping'][key] = m.get(
                            key, self.config['mapping'][key]
                        )
    ...
    def get_controller_config(self, serial):
        # We could support per-controller JSONC in
        # XDG_CONFIG_HOME/pnp/controllers/
        return self.config
