...
class Controller(QObject):
    ...
    def __init__(self, device_path: str, name: str, serial: str | None = None,
                 config=None):
        super().__init__()
    ...
    def _find_battery_device(self):
        ...
        try:
            ...
            logger.debug(f"Could not find battery device for {self.name}: {err}")

    def _update_battery(self):
        if (not self.battery_device_path or
                not os.path.exists(self.battery_device_path)):
        ...
        try:
            ...
            self.battery_changed.emit(
                self.battery_percentage, self.battery_status
            )
        except Exception as err:
            logger.debug(f"Error updating battery for {self.name}: {err}")
    ...
    def start(self):
        ...
        if os.path.lexists(evsieve_link):
            try:
                ...
                logger.debug(
                    f"Failed to remove stale link {evsieve_link}: {err}"
                )

        self.evsieve_proc = ProcessRunner(
            f"evsieve-{self.serial}",
            ["evsieve", "--input", self.device_path, "grab",
             "--output", f"create-link={evsieve_link}"]
        )
        self.evsieve_proc.start()

        self._retry_count = 50
        self._check_timer = QTimer()
        self._check_timer.timeout.connect(
            lambda: self._check_evsieve_link(evsieve_link)
        )
    ...
    def _start_virtual_mapping(self, evsieve_link):
        ...
        self.v_controller = VirtualController(
            name=f"Xbox 360 (PNP-{self.serial})"
        )
        ...
        try:
            ...
            self.notifier = QSocketNotifier(
                self.bridge_device.fd, QSocketNotifier.Read
            )
            self.notifier.activated.connect(self._on_evdev_data)
        except Exception as err:
            logger.error(f"Failed to open evsieve link for bridging: {err}")
            # Retry after a bit
            QTimer.singleShot(
                500, lambda: self._start_virtual_mapping(evsieve_link)
            )
        ...
        try:
            for event in self.bridge_device.read():
                if event.type == e.EV_SYN:
                    self.v_controller.emit(
                        event.type, event.code, event.value, syn=True
                    )
                    continue
            ...
            logger.error(f"Error in bridge data: {err}")
            self.stop()
    ...
    def _cleanup(self):
        ...
        if os.path.lexists(evsieve_link):
            try:
                ...
                logger.debug(f"Failed to remove link {evsieve_link}: {err}")
