...
class VirtualController:
    ...
    def start(self):
        try:
            self.device = evdev.UInput(self.cap, name=self.name, vendor=0x045e, product=0x02a1, version=0x0110)
            logger.info(f"Created virtual controller: {self.name}")
        except Exception as e:
            logger.error(f"Failed to create uinput device: {e}")
            raise

    def emit(self, type, code, value, syn=True):
        if self.device:
            self.device.write(type, code, value)
            if syn:
                self.device.syn()
